...
export class Messages {
  ...
  private readonly dmUserId$ = this.route.paramMap.pipe(map((p) => p.get('dmId')), distinctUntilChanged(), shareReplay({ bufferSize: 1, refCount: true }));
  ...
  private readonly rawMessages$: Observable<MessageBubble[]> = combineLatest([this.currentUser$, this.selectedRecipient$]).pipe(
    switchMap(([user, recipient]) => !user || !recipient ? of([]) : this.directMessagesService.getDirectConversationMessages(user.uid, recipient.uid).pipe(
      map((msgs) => msgs.map((m) => this.mapMessage(m, user)))
    )),
  ...
  constructor() {
    ...
    this.dmUserId$.pipe(
      ...
      switchMap((dmId) => {
        ...
        return from(this.userService.getUserOnce(dmId)).pipe(map((r) => ({ dmId, recipient: r })), catchError((e) => { console.error(e); return of({ dmId, recipient: null }); }));
      })
  ...
  protected async sendMessage(): Promise<void> {
    ...
    this.isSending = true;
    try {
      await this.directMessagesService.sendDirectMessage({ authorId: this.currentUser.uid, authorName: this.currentUser.name, authorProfilePictureKey: this.currentUser.profilePictureKey ?? 'default', text: trimmed }, this.selectedRecipient.uid);
  ...
  private async notifyMentionedRecipient(text: string): Promise<void> {
    ...
    await this.directMessagesService.sendDirectMessage({ authorId: this.selectedRecipient.uid, authorName: Messages.SYSTEM_AUTHOR_NAME, authorProfilePictureKey: Messages.SYSTEM_PROFILE_PICTURE_KEY, text: messageText }, this.selectedRecipient.uid);
  }
  ...
  protected saveEditing(messageId: string): void {
    ...
    this.directMessagesService.updateDirectMessage(this.currentUser.uid, this.selectedRecipient.uid, messageId, { text: trimmed }).finally(() => {
  ...
  protected reactToDmMessage(messageId: string | undefined, emoji: string): void {
    ...
    this.messageReactionsService.toggleReaction({ docPath: `directMessages/${convId}/messages/${messageId}`, userId: this.currentUser.uid, emoji });
